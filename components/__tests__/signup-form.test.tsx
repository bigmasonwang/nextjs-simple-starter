...
describe('SignupForm', () => {
  ...
  it('displays error toast on signup failure', async () => {
    ...
    mockSignUp.mockResolvedValue({ data: null, error: { message: errorMessage } })
  ...
  it('disables signup button while loading', async () => {
    const user = userEvent.setup()
    let resolveSignUp: (value: { data: { user: { id: string } } | null; error: { message: string } | null }) => void
    const signUpPromise = new Promise<{ data: { user: { id: string } } | null; error: { message: string } | null }>((resolve) => {
